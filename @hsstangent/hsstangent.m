            ...
            if ischar(varargin{1})
                switch varargin{1}
                    case 'randn'
                        ...
                        % In theory, we should not need the rank, since we
                        % can extrapolate from the HSS matrix; however, at
                        % the moment we only support constant rank on all
                        % blocks, so we are taking hssrank(...)
                        r = hssrank(H);

                        obj = hsstangent;
                        obj = create_randvec(obj, H, r);
                    case 'zero'
                        H = varargin{2};
                        r = hssrank(H);
                        obj = create_zerovec(obj, H, r);
